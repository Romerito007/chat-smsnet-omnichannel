	...
	return M{
		...
		"InboundMessageMultipart": M{
			...
			"properties": M{
				...
				"group_jid":     describedStr("Group message (multipart): the group JID \"...@g.us\" (same meaning as in the JSON body)."),
				"sender_jid":    describedStr("Group message (multipart): the member's JID."),
				"sender_name":   describedStr("Group message (multipart): the member's display name."),
				"sender_phone":  describedStr("Group message (multipart): the member's phone (digits)."),
				"contacts":      describedStr("Optional: a JSON-encoded array of shared contacts (same shape as the JSON body's contacts[]), so a contact rides a multipart body without a file."),
				"location":      describedStr("Optional: a JSON-encoded location object (same shape as the JSON body's location)."),
				"private":       boolean(),
