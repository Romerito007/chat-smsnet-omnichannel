...
func NewQueryLogRepository(db *mongo.Database) *QueryLogRepository {
	return &QueryLogRepository{coll: db.Collection("monitoring_query_logs")}
}

func (r *QueryLogRepository) Create(ctx context.Context, l *entity.MonitoringQueryLog) error {
	if _, err := shared.RequireTenant(ctx); err != nil {
		return err
	}
	_, err := r.coll.InsertOne(ctx, models.MonitoringQueryLog{
		ID:             l.ID,
		TenantID:       l.TenantID,
		UserID:         l.UserID,
